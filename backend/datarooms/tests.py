...
class TenantLogoApiTests(TestCase):
    """PATCH /api/tenant-config/ et GET /api/tenant-logo/ — logo par office. Repris de
    front/templates-hyperadmin-ui (commit "Logo par office, dernier morceau de la
    marque grise", 02/09/2026), adapté ici à _effective_role uniquement (aucun autre
    changement de comportement) : la couleur était câblée depuis le 28/08, pas le
    logo. Il est stocké sous une clé dans `Office.logo_url` (voir _logo_key : le champ
    porte une clé, pas une URL) et servi par un relais, jamais depuis MinIO — dont
    l'URL est en http quand l'application est en https."""
...
class MfaLoginFlowTests(TestCase):
    ...
    def test_enrollment_flow(self):
        ...
        # Régression : le secret de saisie manuelle doit être en BASE32 (ce qu'une
        # application d'authentification attend), pas en hexadécimal — bug trouvé sur
        # front/templates-hyperadmin-ui (02/09/2026, campagne de test Chrome), corrigé
        # ici le 03/09/2026 (aucun test ne couvrait le FORMAT avant ce lot). Le décodage
        # doit reproduire exactement la clé binaire du dispositif.
        device = TOTPDevice.objects.get(user=self.user, confirmed=False)
        self.assertEqual(base64.b32decode(secret), device.bin_key)
        ...
        good = self.client.post(
            "/api/mfa/setup/",
            {"token": valid_code_for(base64.b32decode(secret).hex())},
            content_type="application/json",
        )
