...
def mfa_setup(request):
    ...
    if request.method == 'GET':
        ...
        # `device.key` est hexadécimal (format de stockage de django-otp) — une
        # application d'authentification attend du base32 pour la saisie manuelle
        # (le QR code, lui, fonctionne déjà : il encode config_url, pas cette valeur
        # directement). Bug trouvé sur front/templates-hyperadmin-ui (02/09/2026,
        # campagne de test Chrome), corrigé ici le 03/09/2026 — voir CLAUDE.md.
        manual_secret = base64.b32encode(device.bin_key).decode('ascii')
        return Response({"qr_code": qr_code_data_uri(device.config_url), "secret": manual_secret})
...
def whoami(request):
    """`is_hyperadmin` (repris de front/templates-hyperadmin-ui, 03/09/2026) : le rôle
...
def _logo_public_url(request, office):
    ...
    Le fichier n'est JAMAIS servi depuis MinIO directement : son URL est en http alors
    que l'application est en https (contenu mixte bloqué), même raison que pour les
    pièces — voir document_content_view. Il passe donc par le relais /api/tenant-logo/.
...
def tenant_config(request):
    """Identité de l'office courant (repris de front/templates-hyperadmin-ui,
    commit "Logo par office", 02/09/2026 — adapté à _effective_role pour que
    l'écriture reste ouverte à un hyperadmin, comme tenant_theme).

    GET   → ouvert à tout membre (ou hyperadmin, _has_office_access) : le nom et le
            logo s'affichent pour chacun.
...
def tenant_logo_view(request):
    ...
    Repris de front/templates-hyperadmin-ui (02/09/2026).

    Ouvert à tout membre (ou hyperadmin), comme la lecture de `tenant_config` : le logo
    s'affiche dans l'en-tête de chacun. Le paramètre `?v=` n'est pas lu, il ne sert qu'à
    distinguer deux logos successifs pour le cache du navigateur."""
    office = request.office
    if office is None:
        return Response({"error": "sous-domaine d'office non résolu"}, status=404)
